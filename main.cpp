...
private:
    ...
    Voxel m_surface;
    Voxel m_visited;
    std::vector<PointIndex>surface_set;
...
{
    PointIndex temp=surface_set[i];
    int indexX=temp.indexX;int indexY=temp.indexY;int indexZ=temp.indexZ;
    double coorX = m_corrX.index2coor(indexX);
    double coorY = m_corrY.index2coor(indexY);
    double coorZ = m_corrZ.index2coor(indexZ);
...
void Model::BFS(int indX, int indY, int indZ){
    ...
    while(!q.empty())
    {
        ...
        fout << coorX << ' ' << coorY << ' ' << coorZ << std::endl;
        //std::cout<<n++<<" "<<v.indexX<<" "<<v.indexY<<" "<<v.indexZ<<std::endl;
        for(int k=0;k<3;k++){
        for(int j=0;j<3;j++){
        for(int i =0;i<3;i++)
        {
            ...
            if(! m_visited[u.indexX][u.indexY][u.indexZ])
            {
                m_visited[u.indexX][u.indexY][u.indexZ]=true;
                if(checkSurface(u.indexX,u.indexY,u.indexZ))
                    q.push(u);
